...
class RozNamchaWindow(QMainWindow, FORM_MAIN):
    ...
    def add_roznamcha(self):
        ...
        if prodcut_id != '' and customer_id != '' and date != '' and quantity != '' and rate != '' and total_amount != '' and cash_paid != '' and cash_received != '':
            ...
            db.conn.execute(f"INSERT INTO customer_cash_received (customer_id,date,description,quantity,rate,amount,cash_paid,cash_received,remaining) VALUES (?,?,?,?,?,?,?,?,?)",(customer_id,date,'roznamcha - ' + str(description),quantity,rate,total_amount,cash_paid,cash_received,customer_remaining))
            db.conn.commit()
