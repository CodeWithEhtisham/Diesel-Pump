...
class MainWindow(QMainWindow, FORM_MAIN):
    ...
    def stock_search_by_date(self):
        ...
        if product == "Select Product":
            print(date)
            data = db.conn.execute(
                f"SELECT stock.date,suppliers.name,stock.stock,stock.rate,stock.amount from stock LEFT JOIN suppliers ON stock.supplier_id=suppliers.supplier_id WHERE date='{date}'").fetchall()
            print('if', data)
            for index, row in enumerate(data):
            ...
            product_id = db.conn.execute(
                f"SELECT product_id FROM products WHERE product_name='{product}'").fetchone()[0]
            data = db.conn.execute(
                f"SELECT stock.date,suppliers.name,stock.stock,stock.rate,stock.amount from stock LEFT JOIN suppliers ON stock.supplier_id=suppliers.supplier_id  WHERE date='{date}' and product_id={product_id}").fetchall()
            print('else')
            for index, row in enumerate(data):
                self.stock_table.insertRow(index)
                for idx, i in enumerate(row):
    ...
    def update(self):
        ...
        data = db.conn.execute(
            "SELECT products.product_name,product_stock,products.uom FROM products").fetchall()
        # print(data)
        ...
        for index, row in enumerate(data):
            ...
            for i in row:
                self.product_table.setItem(
                    index, row.index(i), QTableWidgetItem(str(i)))

        data = db.conn.execute(
            "SELECT stock.date,suppliers.name,stock.stock,stock.rate,stock.amount from stock LEFT JOIN suppliers ON stock.supplier_id=suppliers.supplier_id").fetchall()
        # print(data)
        self.stock_table.setRowCount(0)
        for index, row in enumerate(data):
            self.stock_table.insertRow(index)
        ...
        data = db.conn.execute(
            f"SELECT AVG(rate) FROM stock WHERE date='{current_date}'").fetchone()[0]
    ...
    def stock_search(self):
        ...
        if product == "Select Product":
            ...
            product_id = db.conn.execute(
                f"SELECT product_id FROM products WHERE product_name='{product}'").fetchone()[0]
            data = db.conn.execute(
                f"SELECT stock.date,suppliers.name,stock.stock,stock.rate,stock.amount from stock LEFT JOIN suppliers ON stock.supplier_id=suppliers.supplier_id  WHERE product_id={product_id}").fetchall()
            print('else')
            for index, row in enumerate(data):
                self.stock_table.insertRow(index)
                for idx, i in enumerate(row):
