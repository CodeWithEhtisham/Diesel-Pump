...
class AccountDetailsWindow(QMainWindow, FORM_MAIN):
    ...
    def Handle_Buttons(self):
        ...
        self.btn_search.clicked.connect(self.search_by_date)
        self.btn_refresh.clicked.connect(self.update_account_details)
    ...
    def search(self):
        ...
        data=db.conn.execute(
            f"SELECT date,description,quantity,rate,amount,cash_paid,cash_received,remaining FROM customer_cash_received WHERE customer_id = {self.user_id} AND description LIKE '%{search}%'").fetchall()
    ...
    def search_by_date(self):
        ...
        # get sales record where customer id = self.customer_id
        data=db.conn.execute(
            f"SELECT date,description,quantity,rate,amount,cash_paid,cash_received,remaining FROM customer_cash_received WHERE customer_id = {self.user_id} AND date LIKE '%{date}%'").fetchall()
